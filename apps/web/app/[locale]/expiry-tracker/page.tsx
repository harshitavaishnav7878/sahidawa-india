...
export default function ExpiryTrackerPage() {
  ...
  const getExpiryStatus = (dateStr: string) => {
    ...
    if (diffDays < 0) return { text: "Expired ❌", color: "text-red-600 bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-900/30" };
    if (diffDays <= 30) return { text: `Expiring soon (${diffDays}d) ⚠️`, color: "text-amber-600 bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-900/30" };
    return { text: "Safe ✅", color: "text-emerald-600 bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-900/30" };
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input 
                   ...
                   className="w-full p-3 bg-(--color-surface-page) border border-(--color-border-muted) rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition text-(--color-text-primary)" 
              ...
              <div>
                ...
                <input 
                   ...
                   className="w-full p-3 bg-(--color-surface-page) border border-(--color-border-muted) rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition text-(--color-text-primary) [color-scheme:light] dark:[color-scheme:dark]" 
                />
              </div>
              <div>
                <label className="block text-xs font-bold opacity-60 uppercase tracking-wider mb-1">Batch (Optional)</label>
                <input 
                   type="text" value={batchNumber} onChange={(e) => setBatchNumber(e.target.value)} 
                   className="w-full p-3 bg-(--color-surface-page) border border-(--color-border-muted) rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition text-(--color-text-primary)" 
                   placeholder="B.No-2024" 
                />
              </div>
              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-900/20 transition-all active:scale-95">
                Add to Tracker
              </button>
            </form>
          </div>
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            {!isLoaded ? (
              ...
              <div className="text-center py-20 bg-(--color-surface-muted) border-2 border-dashed border-(--color-border-muted) rounded-3xl opacity-50">
                    ...
                    <div key={med.id} className="p-5 bg-(--color-surface-muted) border border-(--color-border-muted) rounded-2xl shadow-sm flex justify-between items-center hover:border-emerald-500/50 transition-all">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-sm opacity-70">
                          <span className="flex items-center gap-1">📅 {new Date(med.expiryDate).toLocaleDateString('en-IN')}</span>
                          {med.batchNumber && (
                            <span className="bg-(--color-surface-page) px-2 py-0.5 rounded text-[10px] font-mono uppercase border border-(--color-border-muted)">
                              {med.batchNumber}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-4">
                        <span className={`px-4 py-1.5 rounded-full text-[11px] font-bold border ${status.color}`}>
                          {status.text}
                        </span>
                        <button onClick={() => handleDelete(med.id)} className="hover:bg-red-500/10 p-2 rounded-full transition-colors group">
